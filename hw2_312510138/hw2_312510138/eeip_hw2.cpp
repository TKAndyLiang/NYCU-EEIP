...
public:
    ...
    BMP(const string fname, string m, float f, float b, int k) {
        filename = fname;
        mode = m;
        ratio = f;
        beta = b;
...
void BMP::BrightContrastAdjustment(float alpha = 3.0, float beta = 10) {
    ...
    for(int y=0; y<height; y++) {
        for(int x=0; x<width; x++) {
            for(int k=0; k<channels; k++) {
                int offset = y * real_row_width + x * channels + k;
                int pixel = min((int)(alpha * (int)data[offset] + beta), 255);
...
int main(int argc, char** argv) {
    ...
    if(argc == 4) {
        ...
        ss << argv[3]; ss >> f;
        ratio = f;
        ss.clear(); ss << argv[4]; ss >> b;
        beta = b;
    }
    else if(argc == 6) {
        stringstream ss; float f; float b; int k;
        ss << argv[3]; ss >> f;
        ratio = f;
        ss.clear(); ss << argv[4]; ss >> b;
        beta = b;
        ss.clear(); ss << argv[5]; ss >> k;
        kernel_size = k;
    }

    // constructor
    BMP bmp(infile, mode, ratio, beta, kernel_size);
