...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv(x).flatten(2).transpose(1, 2).contiguous()  # B, H//2 * W//2, C_out
        return out
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chan=3, out_chan=3,
        ...
        # Input/Output
        self.input_proj = InputProj(in_channel=in_chan, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        self.output_proj = OutputProj(in_channel=2*embed_dim, out_channel=out_chan, kernel_size=3, stride=1)
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size=384, in_chan=3, out_chan=1,
    ...
    ost = sys.stdout
    macs, params = get_model_complexity_info(model_restoration, (3, 384, 384),
                                             as_strings=True,
                                             print_per_layer_stat=False,
                                             ost=ost)
    
    print('Profiling results using ptflops => ')
    print('{:<30}  {:<8}'.format('Computational complexity: ', macs))
    print('{:<30}  {:<8}'.format('Number of parameters: ', params))


    print('Profiling results using torchinfo => ')
    summary(model_restoration, input_size=(1, 3, 384, 384), col_names=[
                "input_size",
                "output_size",
                "num_params",
                "mult_adds",
